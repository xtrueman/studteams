CREATE USER 'studteams'@'localhost' IDENTIFIED BY 'MySPss123!';
CREATE DATABASE studteams;
GRANT ALL PRIVILEGES ON studteams.* TO 'studteams'@'localhost';

CREATE TABLE `students` (
  `student_id` INT NOT NULL AUTO_INCREMENT COMMENT 'ID студента',
...
CREATE TABLE `teams` (
  `team_id` INT NOT NULL AUTO_INCREMENT COMMENT 'ID команды',
  ...
  PRIMARY KEY (`team_id`),
  CONSTRAINT `teams_ibfk_1` FOREIGN KEY (`admin_student_id`)
...
CREATE TABLE `team_members` (
  ...
  `role` VARCHAR(32) NOT NULL COMMENT 'Роль участника',
...
CREATE TABLE `sprint_reports` (
  ...
  `report_date` TIMESTAMP NOT NULL COMMENT 'Дата/время отправки отчёта',
...
CREATE TABLE `team_members_ratings` (
  ...
  `rate_date` TIMESTAMP NOT NULL COMMENT 'Дата/время выставления оценки',
  ...
  CONSTRAINT `team_members_ratings_ibfk_2` FOREIGN KEY (`assessored_student_id`) REFERENCES `students` (`student_id`)
) COMMENT='Взаимные оценки участников';
