...
def test_config_values_exist():
    ...
    assert hasattr(config, 'MYSQL_USER') 
    assert hasattr(config, 'MYSQL_PASS')
    assert hasattr(config, 'MYSQL_BDNAME')
    
    assert config.MYSQL_HOST
    assert config.MYSQL_USER
    assert config.MYSQL_PASS
    assert config.MYSQL_BDNAME
...
def test_real_connection_success():
    ...
    assert conn.database == config.MYSQL_BDNAME
...
def test_cursor_usage():
    ...
    assert result['current_db'] == config.MYSQL_BDNAME
    dict_cur.close()
